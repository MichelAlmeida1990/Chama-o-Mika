...
const ContasReceber = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingConta) {
        await api.put(`/api/contas-receber/${editingConta.id}/`, formData);
      } else {
        await api.post('/api/contas-receber/', formData);
      }
      setShowModal(false);
      resetForm();
      loadContas();
  ...
  const handleReceber = async (id) => {
    try {
      await api.post(`/api/contas-receber/${id}/receber/`);
      loadContas();
    } catch (error) {
  ...
  const getStatusBadge = (conta) => {
    if (conta.status === 'RECEBIDA') return 'success';
    const hoje = new Date();
